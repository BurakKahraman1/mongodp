...
import {PER_PAGE_FIRST} from "../../_helpers/pagination"
import AllPosts from "../../_queries/AllPosts";
...
export default function Index({mData,posts,currentPageNo}) {
    ...
    return (
        <section className="relative">
            <Head>
                <title>Blog | {mData.companyName}</title>
            </Head>
            <Header/>
                    ...
                    <div className="w-full flex flex-col lg:mr-5 justify-between">
                        ...
                        {posts.pageInfo.offsetPagination.total>=PER_PAGE_FIRST &&
                            <div className="mt-10 mx-auto mb-10 lg:mb-0">
                                <Pagination type="blog" currentPageNo={currentPageNo} postCount={posts.pageInfo.offsetPagination.total}/>
                            </div>
                        }
                    </div>
                    <BlogRightBarMenu/>
...
export async function getServerSideProps() {
    const { data} = await client.query({
        query: AllPosts,
        variables: { size:PER_PAGE_FIRST,offset:0},
    });
    return {
        props: {
            currentPageNo : 1,
